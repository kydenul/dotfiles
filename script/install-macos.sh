#!/usr/bin/env bash
# ============================================================================
# Kyden's Dotfiles - macOS Install Script
# Tested on macOS (Apple Silicon & Intel)
# Usage: bash ~/.dotfiles/script/install-macos.sh
# ============================================================================
...
if [ "$(uname)" != "Darwin" ]; then
    error "This script is designed for macOS. Use install-linux.sh instead."
    exit 1
fi
...
BREW_FORMULAS=(
    ...
    pngpaste    # image paste support (macOS)
    ...
    # Input method
    im-select # input method auto-switching in Neovim (macOS)
...
link_agent_assets() {
    # Shared by all agent CLIs: skills + slash commands
